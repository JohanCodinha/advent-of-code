...
(def input (slurp (io/resource "input-day-7.txt")))
...
(let [parsed (->> (st/split-lines input) 
                  (map parse-rule)
                  (into {}))
      
      can-hold (fn [bags bag-name]
                 (->> bags
